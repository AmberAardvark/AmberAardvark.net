...
.SYNOPSIS
    ...
    Edge browser with CDP, and sets the live-browser base URL.
...
function Get-PackageJson {
    ...
    if (-not (Test-Path $packagePath)) {
        Write-Error "No package.json found in $PSScriptRoot"
        exit 1
    }
...
function Detect-BasePath {
    ...
    foreach ($name in @("astro.config.mjs", "astro.config.ts", "astro.config.js")) {
        $configPath = Join-Path $PSScriptRoot $name
...
if (-not $SkipBrowser) {
    Write-Host ""
    Write-Host "Launching shared Edge browser (CDP port $EdgeDebugPort)..."

    $launchScript = Join-Path $PSScriptRoot "launch-browsers.ps1"
    if (Test-Path $launchScript) {
        & $launchScript -EdgeOnly -Url $Url -EdgeDebugPort $EdgeDebugPort
    } else {
