# Real Browser Toolkit

This toolkit lets you attach Playwright to the same Edge browser window you are already using.

That means you can inspect a live page, click on it, read text from it, watch console and network activity, and take screenshots without switching to a separate automated test browser.

It is designed to be copied into any repo, or moved into its own repo, so you can use the same workflow for websites, userscripts, and browser-extension page behavior.
...
- `CDP` means Chrome DevTools Protocol. It is the connection method that lets this toolkit attach to an existing Edge window.
...
- `launch-browsers.ps1`: starts Edge with the remote-debugging mode that Playwright needs
...
This toolkit supports Chromium-based browsers that expose the Chrome DevTools Protocol, and the included launcher is built for Microsoft Edge.

In practice, you should treat these as the supported paths:

- Microsoft Edge: supported and included in the launcher scripts
- Google Chrome or another Chromium-based browser: may work if you launch it yourself with a compatible CDP port and point the toolkit at it
...
Open PowerShell in the `real-browser-toolkit` folder and run:
...
- Edge was not started through `launch-browsers.ps1` or `start-dev.ps1`
- Edge is already open in a conflicting state
- the remote debugging port is not the expected one

If you are using a browser other than Edge, the same rule applies: it needs to be launched by this toolkit, or manually launched with a compatible CDP port.

Try closing Edge and running the launcher again.
...
Then bring the desired tab to the front in Edge and run the command again.

## Notes

- Microsoft Edge is the supported out-of-the-box browser for this bundle.
- Other Chromium-based browsers may work if you launch them manually with a compatible CDP endpoint.
